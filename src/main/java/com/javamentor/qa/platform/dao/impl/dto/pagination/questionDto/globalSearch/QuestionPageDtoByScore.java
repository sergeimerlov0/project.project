...
public class QuestionPageDtoByScore implements PaginationDtoAble<QuestionViewDto> {
    ...
    public List<QuestionViewDto> getItems(Map<String, Object> param) {
        int score1 = (int) param.get("score1");
        int score2 = (int) param.get("score2");
        ...
        return entityManager.createQuery("SELECT DISTINCT new com.javamentor.qa.platform.models.dto.QuestionViewDto" +
                        ...
                        "LEFT JOIN question.user WHERE question.voteQuestions.size >= :score1 AND question.voteQuestions.size <= :score2 AND question.isDeleted = false ORDER BY question.id", QuestionViewDto.class)
    ...
    public int getTotalResultCount(Map<String, Object> param) {
        ...
        return Math.toIntExact((Long) entityManager.createQuery("SELECT COUNT(DISTINCT question.id) FROM Question question LEFT JOIN question.user " +
                        "WHERE question.voteQuestions.size >= :score1 AND question.voteQuestions.size <= :score2 AND question.isDeleted = false")
                .setParameter("score1", score1)
                .setParameter("score2", score2)
                .getSingleResult());
    }
